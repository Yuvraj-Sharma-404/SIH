...
export default function NodalOfficersPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="gov-card p-6 sm:p-8 bg-white space-y-5">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-200 pb-4">
          <div className="flex items-center space-x-2 bg-slate-100 p-1 rounded-xl border border-slate-200">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-bold transition ${
                activeTab === "central"
                  ? "bg-[#001c5a] text-white shadow-sm"
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 rounded-lg border border-slate-300 text-xs focus:outline-none focus:border-gov-navy focus:ring-1 focus:ring-gov-navy"
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto border border-slate-200 rounded-xl">
          <table className="w-full text-left text-xs text-slate-700 border-collapse">
            <thead className="bg-[#001c5a] text-white uppercase text-[11px] font-bold">
              <tr>
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {filteredList.map((item, idx) => (
                <tr key={item.sno} className="hover:bg-slate-50 transition">
                  <td className="py-3 px-3 text-center font-bold text-slate-500">{idx + 1}</td>
                  <td className="py-3 px-4 font-extrabold text-gov-navy">{item.org}</td>
                  <td className="py-3 px-4">
                    <span className="font-bold text-slate-900 block">{item.officer}</span>
                    <span className="text-[11px] text-slate-500">{item.designation}</span>
                  </td>
                  <td className="py-3 px-4 text-slate-600 text-[11px] leading-tight max-w-xs">
                    {item.address}
                  </td>
                  <td className="py-3 px-4 space-y-0.5">
                    <div className="font-mono text-slate-800 font-semibold">{item.phone}</div>
                    <div className="text-gov-navy font-medium text-[11px]">{item.email}</div>
                  </td>
                </tr>
              ))}
