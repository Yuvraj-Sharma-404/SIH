...
function TrackComplaintContent() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 py-2">
      {/* Header */}
      <div className="gov-card p-6 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/95 gov-border-t-navy space-y-4 transition-colors">
        <div>
          <span className="text-xs uppercase font-mono font-bold text-gov-saffron">
            SmadhanX Standard Workflow
          </span>
          <h1 className="text-2xl font-bold text-gov-navy dark:text-sky-400 font-serif mt-0.5">
        ...
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-2 pt-1">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-sm focus:outline-none focus:border-gov-navy dark:focus:border-sky-400 font-mono transition-colors"
            />
          </div>
          <button
            type="submit"
            disabled={loading}
            className="px-6 py-2.5 rounded-lg bg-gov-navy hover:bg-gov-navy-dark dark:bg-sky-600 dark:hover:bg-sky-500 text-white font-bold text-xs shadow-sm transition flex items-center justify-center space-x-1.5 cursor-pointer"
          >
      ...
      {loading ? (
        <div className="py-20 text-center space-y-3">
          <Loader2 className="w-8 h-8 text-gov-navy dark:text-sky-400 animate-spin mx-auto" />
          <p className="text-xs text-slate-500 dark:text-slate-400 font-mono">Retrieving SmadhanX record...</p>
        </div>
      ) : !hasSearched ? (
        <div className="gov-card p-12 text-center space-y-3 bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 transition-colors">
          <FileText className="w-10 h-10 text-slate-400 dark:text-slate-500 mx-auto" />
          <h3 className="text-base font-bold text-slate-800 dark:text-white">Track Grievance Status</h3>
        ...
        <div className="gov-card p-12 text-center space-y-3 bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 transition-colors">
          ...
          <h3 className="text-base font-bold text-slate-900 dark:text-white">No Grievance Found</h3>
        ...
        <div className="space-y-6">
          ...
          <div className="gov-card p-6 bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 space-y-4 transition-colors">
            <div className="flex flex-wrap items-start justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-700">
              <div>
                <div className="flex items-center space-x-2">
                  <span className="font-mono text-xs font-extrabold px-2.5 py-0.5 rounded bg-blue-50 dark:bg-sky-950/50 text-gov-navy dark:text-sky-300 border border-blue-200 dark:border-sky-800">
                ...
                <h2 className="text-xl font-bold text-gov-navy dark:text-sky-400 mt-2 font-serif">
                  {complaint.title}
                </h2>
                <div className="flex flex-wrap items-center gap-4 text-xs text-slate-600 dark:text-slate-300 mt-1.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2 border-t border-slate-100 dark:border-slate-700 text-xs">
              <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700">
                ...
                <p className="font-bold text-gov-navy dark:text-sky-400 mt-0.5">
              ...
              <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700">
                ...
                <p className="font-bold text-gov-saffron mt-0.5 font-mono">
                  ...
                  {complaint.priorityScore != null && (
                    <span className="text-[10px] font-sans font-medium text-slate-600 dark:text-slate-400 ml-1.5">
          ...
          <div className="gov-card p-6 bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 space-y-6 transition-colors">
                ...
                return (
                  <div key={stage.key} className="relative">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                      ...
                      {isCurrent && (
                        <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-orange-100 dark:bg-orange-950/60 text-orange-800 dark:text-orange-300 border border-orange-200 dark:border-orange-800 self-start sm:self-auto">
                          Active Stage
                        </span>
                      )}
                      {isCompleted && (
          ...
          <div className="gov-card p-6 bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 space-y-4 transition-colors">
            ...
            {feedbackSubmitted || complaint.status === "CLOSED" ? (
              <div className="p-4 rounded-lg bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 text-xs flex items-center space-x-2">
              ...
              <form onSubmit={submitCitizenFeedback} className="space-y-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs text-slate-600 dark:text-slate-300 font-semibold ml-2">
                ...
                <textarea
                  ...
                  className="w-full p-3 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-xs focus:outline-none focus:border-gov-navy dark:focus:border-sky-400 transition-colors"
                />

                <button
                  type="submit"
                  disabled={submittingFeedback}
                  className="px-5 py-2.5 rounded-lg bg-gov-emerald hover:bg-emerald-700 text-white text-xs font-bold transition flex items-center space-x-1.5 shadow-sm cursor-pointer"
                >
                  {submittingFeedback ? (
