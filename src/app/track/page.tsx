...
export default function TrackComplaintPage() {
  ...
  const initialId = searchParams.get("id") || "PS-2026-1042";
  ...
  const loadComplaint = async (id: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/problems/${id.trim()}`);
      ...
      <div className="gov-card p-6 rounded-xl border border-slate-200 bg-white gov-border-t-navy space-y-4">
        ...
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-2 pt-1">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Enter Registration / Complaint ID (e.g. PS-2026-1042)"
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 text-xs text-slate-500">
          ...
          <button
            type="button"
            onClick={() => {
              setComplaintId("PS-2026-1042");
              loadComplaint("PS-2026-1042");
            }}
            className="px-2.5 py-1 rounded bg-slate-100 hover:bg-slate-200 border border-slate-300 font-mono text-gov-navy font-bold"
          >
            PS-2026-1042 (Bridge Pier Damage)
          </button>
          <button
            type="button"
            onClick={() => {
              setComplaintId("PS-2026-2189");
              loadComplaint("PS-2026-2189");
            }}
            className="px-2.5 py-1 rounded bg-slate-100 hover:bg-slate-200 border border-slate-300 font-mono text-gov-navy font-bold"
          >
            PS-2026-2189 (Water Contamination)
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="gov-card p-12 text-center space-y-3 bg-white border border-slate-200">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            No record found matching "{complaintId}". Please verify the tracking number or try one of the demonstration records above.
          </p>
        </div>
      ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2 border-t border-slate-100 text-xs">
              <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                ...
                <p className="font-bold text-gov-navy mt-0.5">
                  {complaint.departmentName || "Public Works Department (PWD)"}
                </p>
              </div>
              ...
              <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                ...
                <p className="font-bold text-slate-800 mt-0.5">
                  {complaint.assignedOfficer || "Er. A. K. Sharma (Exec. Engineer)"}
                </p>
              </div>
              ...
              <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                ...
                <p className="font-bold text-gov-saffron mt-0.5 font-mono">
                  {complaint.priorityScore} / 100 (High Priority)
                </p>
              </div>
            </div>
          </div>
