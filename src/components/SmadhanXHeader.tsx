...
export default function SmadhanXHeader() {
  ...
  React.useEffect(() => {
    async function checkAuth() {
      // 1. Immediately read cached user for instant zero-delay UI render
      try {
        const saved = localStorage.getItem("smadhanx_user");
        if (saved) {
      ...
      // 2. Validate session against server with anti-cache headers
      try {
        ...
        // network issue - retain persistent state
      }
    }
  ...
  const handleSignOut = async () => {
    try {
      localStorage.removeItem("smadhanx_user");
    ...
    <>
      ...
      <nav className="bg-[#001c5a] text-white text-xs font-semibold shadow-md sticky top-0 z-40">
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden xl:flex items-center justify-between min-h-[52px] py-1 gap-2">
            {/* Left Nav Menu Items */}
            <div className="flex items-center space-x-0.5 xl:space-x-1 text-[11.5px]">
            ...
            <div className="shrink-0 flex items-center gap-3 ml-auto pl-4 border-l border-white/20 flex-nowrap min-w-max">
              {/* Language Selector Dropdown */}
              <div className="relative group shrink-0">
                <button
                  type="button"
                  className="flex items-center space-x-1.5 px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 border border-white/20 transition text-[11px] whitespace-nowrap"
                >
              ...
              {currentUser ? (
                <div className="shrink-0 flex items-center gap-2 flex-nowrap min-w-max">
                  <button
                    type="button"
                    onClick={() => setIsProfileModalOpen(true)}
                    className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-white font-medium text-xs transition border border-white/20 shadow-xs whitespace-nowrap cursor-pointer hover:border-gov-saffron/70 group"
                    title="Click to view full user profile details"
                  >
                    <span className="w-5 h-5 rounded-full bg-gov-saffron/20 border border-gov-saffron flex items-center justify-center text-[10px] font-bold text-gov-saffron group-hover:scale-110 transition-transform">
                      {currentUser.name.charAt(0).toUpperCase()}
                    </span>
                    <span className="max-w-[110px] truncate font-semibold">{currentUser.name.split(" ")[0]}</span>
                  </button>

                  <button
                    type="button"
                    onClick={handleSignOut}
                    className="shrink-0 flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-red-600 hover:bg-red-700 text-white font-bold text-xs transition shadow-sm whitespace-nowrap active:scale-95 min-w-max"
                ...
                <Link
                  href="/login"
                  className="shrink-0 inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-gov-saffron hover:bg-orange-600 text-white font-bold text-xs transition-all shadow-md hover:shadow-orange-500/20 active:scale-95 whitespace-nowrap min-w-max tracking-wide"
                >
                  <img src="/Images/signIn.png" alt="" className="w-3.5 h-3.5 object-contain shrink-0" />
                ...
                {currentUser ? (
                  <>
                    <button
                      ...
                      className="flex-1 py-2 px-3 bg-white/15 hover:bg-white/25 rounded text-center text-xs font-bold text-white flex items-center justify-center gap-1.5"
                    >
                      <User className="w-3.5 h-3.5 text-gov-saffron shrink-0" />
                      <span className="truncate">{currentUser.name.split(" ")[0]}'s Profile</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="py-2 px-3 bg-red-600 hover:bg-red-700 rounded text-center text-xs font-bold text-white flex items-center justify-center gap-1.5 shrink-0"
                    >
                      <LogOut className="w-3.5 h-3.5 shrink-0" />
                      <span>Sign&nbsp;Out</span>
                    </button>
                  ...
                  <Link
                    href="/login"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex-1 py-2 px-3 bg-gov-saffron hover:bg-orange-600 rounded text-center text-xs font-bold text-white whitespace-nowrap inline-flex items-center justify-center"
                  >
                    <span>Sign&nbsp;In</span>
                  </Link>
                )}
